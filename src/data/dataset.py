...
class SourceSeparationDataset(Dataset):
    ...
    def __init__(
        ...
        if txt_path is None and txt_dir is not None:
            self.mode = "train" if self.is_training else "valid"
            self.txt_path = Path(txt_dir) / f"{target}_{self.mode}.txt"
        ...
        self.filelist, self.audiolist = self.get_filelist()
    ...
    def get_filelist(self) -> tp.List[tp.Tuple[str, tp.Tuple[int, int]]]:
        ...
        for line in tqdm(open(self.txt_path, "r").readlines()):
            ...
            if file_name not in filename2label:
                ...
                i += 1
            filepath_template = self.file_dir / self.mode / f"{file_name}" / "{}.wav"
            if self.preload_dataset:
                mix_segment, tgt_segment = self.load_files(
                    str(filepath_template), (int(start_idx), int(end_idx))
                )
                audiolist.append((mix_segment, tgt_segment))
            filelist.append((str(filepath_template), (int(start_idx), int(end_idx))))
        return filelist, audiolist

    def load_file(self, file_path: str, indices: tp.Tuple[int, int]) -> torch.Tensor:
        assert Path(file_path).is_file(), f"There is no such file - {file_path}."

        offset = indices[0]
        num_frames = indices[1] - indices[0]
        y, sr = torchaudio.load(
            file_path, frame_offset=offset, num_frames=num_frames, channels_first=True
        )
    ...
    def load_files(
        ...
        mix_segment = self.load_file(fp_template.format("mixture"), indices)
        tgt_segment = self.load_file(fp_template.format(self.target), indices)
    ...
    def imitate_silent_segments(
        mix_segment: torch.Tensor, tgt_segment: torch.Tensor
    ) -> tp.Tuple[torch.Tensor, torch.Tensor]:
        return (mix_segment - tgt_segment, torch.zeros_like(tgt_segment))
    ...
    def mix_segments(
        ...
        targets_to_add = random.sample(self.TARGETS, n_sources)
    ...
    def augment(
        self, mix_segment: torch.Tensor, tgt_segment: torch.Tensor
    ) -> tp.Tuple[torch.Tensor, torch.Tensor]:
        if self.is_training:
            # mixing with other sources
            if random.random() < self.mix_prob:
                mix_segment, tgt_segment = self.mix_segments(tgt_segment)
            # dropping target
            # random값이 0.1보다 작으면 (즉 0.1의 확률) target을 zero로 두고 입력도 target을 뺌
    ...
    def __getitem__(self, index: int) -> tp.Tuple[torch.Tensor, torch.Tensor]:
        ...
        # load files
        if self.preload_dataset:
            mix_segment, tgt_segment = self.audiolist[index]
        else:
...
class EvalSourceSeparationDataset(Dataset):
    ...
    def get_test_filelist(self) -> tp.List[tp.Tuple[str, str]]:
        ...
        for fp in test_dir.glob("*"):
            ...
            fp_tgt = [fp_template.format(tgt_instr) for tgt_instr in self.target]
    ...
    def get_inference_filelist(self) -> tp.List[tp.Tuple[str, str]]:
        ...
        if self.in_fp.is_file() and self.in_fp.suffix in self.EXTENSIONS:
            ...
            for in_fp in self.in_fp.glob("*/*"):
                ...
                # directory structure must be 'some identifier/mixture.wav'
                if "mixture" in str(in_fp):

                    out_fp = self.out_fp / in_fp.parent.name
    ...
    def load_file(self, file_path: str) -> torch.Tensor:
        assert Path(file_path).is_file(), f"There is no such file - {file_path}."
        y, sr = torchaudio.load(file_path, channels_first=True)
        if sr != self.sr:
            y = torchaudio.transforms.Resample(orig_freq=sr, new_freq=self.sr)(y)
    ...
    def __getitem__(
        ...
        if self.mode == "test":
            y_tgt = torch.stack([self.load_file(fp_instr) for fp_instr in fp_tgt])
            # y_tgt = self.load_file(fp_tgt)
