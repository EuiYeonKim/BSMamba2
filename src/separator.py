...
class Separator(nn.Module):
    def __init__(
        ...
        self.featurizer, self.model, self.inverse_featurizer = self.initialize_modules()
        ...
        pad_chunk = (
            self.featurizer.hop_length - (self.chunk_size % self.featurizer.hop_length)
        ) % self.featurizer.hop_length
    ...
    def initialize_modules(self) -> nn.Module:
        ...
        # load modules
        featurizer, inverse_featurizer, _ = initialize_featurizer(self.cfg)
        ...
        return featurizer, model, inverse_featurizer
    ...
    def separate(self, y: torch.Tensor) -> torch.Tensor:
        ...
        for s, e in get_minibatch(n_chunks, self.bs):
            ...
            x = self.featurizer(y[s:e])
            chunk = self.model(x)
            chunk = self.inverse_featurizer(chunk)
            if window is None:
    ...
    def fold(self, y: torch.Tensor, n_channels: int, duration: int) -> torch.Tensor:
        ...
        y_out = torch.zeros((n_sources, n_channels, duration)).to(y)

        # overlap-add
        for i in range(n_chunks):
            y_out[..., start : start + self.ws] = (
